...
class TestCase(unittest.TestCase):
    ...
    def __buildBar(self, openPrice, highPrice, lowPrice, closePrice):
        date_time = datetime.datetime.now() + datetime.timedelta(seconds=self.__currSeconds)
        self.__currSeconds += 1
        return bar.Bar(date_time, openPrice, highPrice, lowPrice, closePrice, closePrice*10, closePrice)
    ...
    def __buildBarDataSeries(self, closePrices, highPrices, lowPrices):
        ...
        for i in range(len(highPrices)):
            ret.append_value( self.__buildBar(closePrices[i], highPrices[i], lowPrices[i], closePrices[i]) )
        return ret
