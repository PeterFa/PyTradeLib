...
class QueueStrategyExecutionForm(forms.Form):
    ...
    def __init__(self, request, strategyParams):
        ...
        fieldNames.append(QueueStrategyExecutionForm.symbolParam)
    ...
    def get_symbol(self):
        return self.getRawValue(QueueStrategyExecutionForm.symbolParam).strip()
    ...
    def getTemplateValues(self):
        ...
        # Other values.
        ret[QueueStrategyExecutionForm.symbolParam] = formValues[QueueStrategyExecutionForm.symbolParam]
    ...
    def loadFromStratExecConfig(self, stratExecConfig):
        ...
        # Other values.
        dateFormat = "%Y/%m/%d"
        self.setRawValue(QueueStrategyExecutionForm.symbolParam, str(stratExecConfig.symbol))
...
class StrategyPage(webapp.RequestHandler):
    ...
    @staticmethod
    def getUrl(strategy_className):
        return utils.build_url(StrategyPage.url, {"class":strategy_className})

    def get(self):
        templateValues = {}
        strategy_className = cgi.escape(self.request.get("class")).strip()

        # Try to load the class.
        try:
            strategy_class = cls.Class(strategy_className)
        except Exception, e:
            templateValues["error"] = "Failed to load strategy '%s': %s" % (strategy_className, e)
            path = os.path.join(os.path.dirname(__file__), "..", "templates", 'error.html')
            self.response.out.write(template.render(path, templateValues))
            return

        # persistence.StratExecConfig.getByClass(strategy_className)

        # Template values
        strategyValues = {}
        strategyValues["class"] = strategy_className
        ...
        templateValues["queue_execution_url"] = StrategyExecutionPage.getUrl(strategy_className)

        templateValues["active_executions"] = get_stratexecconfig_for_template(persistence.StratExecConfig.getByClass(strategy_className, [persistence.StratExecConfig.Status.ACTIVE]))
        templateValues["finished_executions"] = get_stratexecconfig_for_template(persistence.StratExecConfig.getByClass(strategy_className, [persistence.StratExecConfig.Status.FINISHED, persistence.StratExecConfig.Status.CANCELED_TOO_MANY_ERRORS]))
...
class StrategyExecutionPage(webapp.RequestHandler):
    ...
    @staticmethod
    def getUrl(strategy_className):
        return utils.build_url(StrategyExecutionPage.url, {"class":strategy_className})

    @staticmethod
    def getReExecuteUrl(strategy_className, stratExecConfigKey):
        return utils.build_url(StrategyExecutionPage.url, {
            "class":strategy_className,
    ...
    def __handleRequest(self, isPost):
        templateValues = {}
        strategy_className = cgi.escape(self.request.get("class"))
        ...
        try:
            strategyParams = strategy_class.getMethodParams("__init__")
            minParamCount = 3
            ...
            templateValues["error"] = "Failed to get __init__ parameters for '%s': %s" % (strategy_className, e)
            path = os.path.join(os.path.dirname(__file__), "..", "templates", 'error.html')
            self.response.out.write(template.render(path, templateValues))
            return
        ...
        if isPost:
            try:
                ...
                # Check that we have bars loaded for the given symbol within the given dates.
                symbol = form.get_symbol()
                ...
                if not persistence.Bar.hasBars(symbol, persistence.Bar.Type.DAILY, beginDate, endDate):
                    raise Exception("There are no bars loaded for '%s' between '%s' and '%s'" % (symbol, beginDate, endDate))

                # Queue the strategy execution config and redirect.
                stratExecConfig = self.__buildStratExecConfig(strategy_className, strategyParams, form)
                stratExecConfig.put()
                seproducer.SEProducerHandler.queue(stratExecConfig.key())
                self.redirect(StrategyPage.getUrl(strategy_className))
            except Exception, e:
